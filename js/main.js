(() => {
  ...
  const profilePhoto = document.getElementById('profile-photo'); const showProfilePhoto = () => document.getElementById('profile-slot')?.classList.add('has-image'); if (profilePhoto?.complete && profilePhoto.naturalWidth > 0) showProfilePhoto(); profilePhoto?.addEventListener('load', showProfilePhoto); profilePhoto?.addEventListener('error', () => profilePhoto.remove());
